...
function isFlood(id) {
  ...
  if (blockList[id] && blockList[id] > now) {
    bot.sendMessage(id, formatMessage('БЛОКИРОВКА', '⛔ Ты заблокирован на 1 час за подозрительную активность.'));
    return true;
  }
...
console.log('🏴‍☠️ ЧЁРНАЯ КОСТЬ v7.0 ЗАПУЩЕНА');
console.log(`👥 Игроков: ${Object.keys(players).length}`);
console.log(`💰 Банк: ${safeNumber(bank.pot)}, Джекпот: ${safeNumber(bank.jackpot)}`);
console.log('✅ ВСЕ ДОРАБОТКИ РЕАЛИЗОВАНЫ:');
console.log('   - Новый дизайн сообщений (formatMessage)');
console.log('   - Достижения дают % к пассивному доходу');
console.log('   - Джекпот пополняется случайным % от ставки');
console.log('   - Кнопки "Играть ещё" и "Главное меню" после игр');
console.log('   - Раздельные клавиатуры для каждого раздела');
console.log('   - Кнопка "Назад" во всех информационных меню');
