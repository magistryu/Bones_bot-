...
function fleetKeyboard() {
  const keyboard = [];
  // Показываем ВСЕ корабли, а не только 10
...
const balance = data.demoMode ? safeNumber(data.demoBalance) : safeNumber(data.balance);
  ...
  // Получаем ID корабля из callback_data
  const shipId = parseInt(data.split('_')[2]);
  if (isNaN(shipId)) {
    bot.sendMessage(id, '❌ Ошибка ID корабля.');
    return;
  }

  // Находим корабль в списке
  ...
  // Проверяем, есть ли у игрока флот
  if (!p.fleet) {
    p.fleet = { ships: [], totalIncome: 0, lastCollected: Date.now() };
  }

  // Проверяем, не куплен ли уже этот корабль
  if (p.fleet.ships.some(s => s.id === shipId)) {
    bot.sendMessage(id, `❌ Корабль "${ship.name}" уже куплен!`);
    return;
  }
  ...
  if (balance < ship.cost) {
    bot.sendMessage(id, `❌ Не хватает дублонов! Нужно ${ship.cost}, у тебя ${balance}.`);
    return;
  }
  ...
  bot.sendMessage(id,
    `✅ ПОЗДРАВЛЯЮ! Ты купил корабль "${ship.name}"!\n\n` +
  ...
  if (data.startsWith('roll_classic_')) {
    ...
    if (playerDice === playerDice2) {
      ...
      // Сохраняем состояние точки
      saveData();

      // Показываем сообщение о точке
      bot.sendMessage(id, `🎯 ТОЧКА! ${playerDice}+${playerDice2}=${playerSum} очков!\n\n` +
        `💰 Ставка: ${amount} дуб.\n` +
        `🎯 Множитель: x1\n\n` +
        `Выбери действие:`, {
        reply_markup: {
  ...
  if (data.startsWith('point_take_')) {
    ...
    if (!p.point || !p.pointBet || p.pointBet !== amount) {
      bot.sendMessage(id, '❌ Точка не найдена. Начни игру заново.');
      return;
    }
  ...
  if (data.startsWith('point_double_')) {
    ...
    if (playerDice === playerDice2) {
      ...
      bot.sendMessage(id, `🎯 НОВАЯ ТОЧКА! ${playerDice}+${playerDice2}=${playerSum} очков!\n\n` +
        `💰 Ставка: ${amount} дуб.\n` +
      ...
      bot.sendMessage(id, `💀 УДВОЕНИЕ ПРОВАЛИЛОСЬ!\n\n` +
...
});   // ← ЭТА СТРОКА ЗАКРЫВАЕТ ВЕСЬ callback_query
...
bot.on('message', async (msg) => {
  ...
  return;
    ...
    if (p.currentMode === 'vip') {
     ...
     p.currentMode = null;      // ← ДОБАВИТЬ ЭТУ СТРОКУ
     delete p.tempBet;
     saveData();
     return;
    }
