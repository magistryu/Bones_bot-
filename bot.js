...
const EVENTS = [
  ...
  { name: '🎲 Удача пирата', desc: 'Игрок может перебросить кубик 1 раз (в любой игре)', duration: 1800000 },
  ...
  { name: '🌊 Пиратский бриз', desc: 'Все ставки возвращаются при поражении (1 час)', duration: 3600000 },
  ...
  { name: '💀 Проклятие Чёрной кости', desc: 'Проигрыш уменьшен на 30%, выигрыш увеличен на 10%', duration: 3600000 },
  ...
  { name: '💀 Череп и кости', desc: 'Проигрыш уменьшен на 40%, выигрыш увеличен на 20%', duration: 3600000 },
...
function getRandomEvent() {
  const available = EVENTS.filter(e => !usedEvents.includes(e.name));
  ...
  setTimeout(() => {
    ...
    for (let id in players) {
      ...
      if ((balance > 0 || safeNumber(p.demoBalance) > 0) && (Date.now() - p.lastPassiveTime < 86400000)) {
        bot.sendMessage(id, formatMessage(
          '🎉 НОВОЕ СОБЫТИЕ!',
          `${event.name}\n${event.desc}\n\n⏳ Длится: ${event.duration / 60000} минут\n📊 Осталось событий в пуле: ${EVENTS.length - usedEvents.length}`
        )).catch(() => {});
      }
    }
    bot.sendMessage(ADMIN_ID, formatMessage(
      '📢 СОБЫТИЕ ЗАПУЩЕНО',
      `${event.name}\n${event.desc}\n⏳ Длится: ${event.duration / 60000} минут\n📊 Осталось событий: ${EVENTS.length - usedEvents.length}/${EVENTS.length}`
    ));
...
// ==================== ФУНКЦИЯ ОТПРАВКИ СТИКЕРОВ С АНИМАЦИЕЙ ====================
async function sendDiceAnimation(chatId, playerSum, bankSum) {
  // Шаг 1: Стакан трясётся
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('roll_classic_')) {
    ...
    // Выбираем фразу в зависимости от результата
    let phrase = '';
    let resultTitle = '';
    let resultEmoji = '';
    if (winAmount > 0) {
...
console.log('🏴‍☠️ ЧЁРНАЯ КОСТЬ v10.0 — ФИНАЛЬНАЯ ВЕРСИЯ ЗАПУЩЕНА');
console.log(`👥 Игроков: ${Object.keys(players).length}`);
console.log(`💰 Банк: ${safeNumber(bank.pot)}, Джекпот: ${safeNumber(bank.jackpot)}`);
console.log('📦 Стикеры с кубиками активированы');
console.log('🎯 Все 16 пунктов доработок реализованы!');
